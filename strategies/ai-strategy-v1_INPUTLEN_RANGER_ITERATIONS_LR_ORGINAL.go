...
func NewAIStrategyV1ORG(input_len int, ranger int, iterations int, lr float64, config ffnnDomain.Config) domain2.IStrategyModel {
...
func (st *AIStrategyV1ORG) Init(base_asset, trade_asset, quote_asset string, binanceClients []binanceDomain.IBinanceClient, candles []domain.Candle, is_ready func(mapName string, is_ready bool)) {
...
func (st *AIStrategyV1ORG) OnCandle(candle domain.Candle) {
	pair := fmt.Sprintf("%s%s", st.QuoteAsset, st.BaseAsset)
	var examples ffnnDomain.Examples
	var prediction int
	if len(st.candles) > 0 {
...
func (st *AIStrategyV1ORG) UpdateBinanceClients(binance_clients []binanceDomain.IBinanceClient) {
	if st.strategy == nil {
